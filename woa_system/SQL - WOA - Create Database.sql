...
GO
IF OBJECT_ID ( 'WOA.getWorkflowStatuses', 'P' ) IS NOT NULL
	DROP PROCEDURE WOA.getWorkflowStatuses;
GO
IF OBJECT_ID ('WOA.getWorkorderWorkflow', 'P' ) IS NOT NULL
	DROP PROCEDURE WOA.getWorkorderWorkflow;
GO
...
GO
IF OBJECT_ID ( 'WOA.getCommitmentProjects', 'P' ) IS NOT NULL
	DROP PROCEDURE WOA.getCommitmentProjects;
GO
...
GO
IF OBJECT_ID ( 'DPSS.getCOTypes', 'P' ) IS NOT NULL
	DROP PROCEDURE DPSS.getCOTypes;
GO
...
GO
IF OBJECT_ID ( 'WOA.mergeEmployee', 'P' ) IS NOT NULL
	DROP PROCEDURE WOA.mergeEmployee;
GO
...
GO
IF OBJECT_ID ( 'WOA.getWorkActivities', 'P' ) IS NOT NULL
	DROP PROCEDURE WOA.getWorkActivities;
GO
IF OBJECT_ID ( 'SAP.mergeEmployee', 'P' ) IS NOT NULL
	DROP PROCEDURE SAP.mergeEmployee;
GO
...
GO
IF OBJECT_ID('WOA.WorkActivity','U') IS NOT NULL
	DROP TABLE WOA.WorkActivity;
GO
IF OBJECT_ID('WOA.ParentOrderProject') IS NOT NULL
	DROP TABLE WOA.ParentOrderProject;
GO
IF OBJECT_ID('WOA.WorkOrderWorkflow','U') IS NOT NULL
	DROP TABLE WOA.WorkOrderWorkflow;
GO
...
GO
IF OBJECT_ID('WOA.WorkflowStatus','U') IS NOT NULL
	DROP Table WOA.WorkflowStatus;
GO
IF OBJECT_ID('WOA.CommitmentProject','U') IS NOT NULL
	DROP TABLE WOA.CommitmentProject;
GO
...
-- WOA.WorkActivity
PRINT '-- WOA.WorkActivity';
GO
CREATE TABLE WOA.WorkActivity
(
	WorkActivityID [nchar](1) NOT NULL PRIMARY KEY,
	Name [dbo].[Name] NOT NULL,
	CreatedBy [nvarchar](50) NOT NULL DEFAULT USER_NAME(),
	CreatedDate [datetime2](2) NOT NULL DEFAULT SYSUTCDATETIME(),
	LastUpdatedBy [nvarchar](256) NULL,
...
EXEC sys.sp_addextendedproperty 
	 @name=N'MS_Description', 
	 @value=N'Work Activity Types', 
	 @level0type=N'SCHEMA', @level0name=N'WOA', 
	 @level1type=N'TABLE',  @level1name=N'WorkActivity';
GO
...
-- WOA.CommitmentProject
PRINT '-- WOA.CommitmentProject';
GO
CREATE TABLE WOA.CommitmentProject
(
	CommitmentProjectID [int] IDENTITY(1,1) NOT NULL PRIMARY KEY,
	Name [dbo].[Name] NULL,
...
(
	...
	CommitmentProjectID [INT] NOT NULL	DEFAULT 1
										CONSTRAINT FK_WOAWorkOrder_CommitmentProjectID_CommitmentProject
										FOREIGN KEY (CommitmentProjectID)
										REFERENCES WOA.CommitmentProject(CommitmentProjectID),
...
(
	WorkOrderHistoryID [int] IDENTITY(1,1) NOT NULL PRIMARY KEY,
...
-- WOA.WorkflowStatus
PRINT '-- WOA.WorkflowStatus';
GO
CREATE TABLE WOA.WorkflowStatus
(
	WorkflowStatusID [int] IDENTITY(1,1) NOT NULL PRIMARY KEY,
	Name [dbo].[name] NULL,
	Revision [tinyint] NOT NULL DEFAULT 0,
	CreatedBy [nvarchar](256) NOT NULL DEFAULT USER_NAME(),
	CreatedDate datetime2 NOT NULL DEFAULT SYSUTCDATETIME(),
...
EXEC sys.sp_addextendedproperty
	@name=N'MS_Description',
	@value=N'Workflow Status Options',
	@level0type=N'SCHEMA', @level0name=N'WOA',
	@level1type=N'TABLE', @level1name=N'WorkflowStatus';
GO

-- WOA.WorkOrderWorkflow
PRINT '-- WOA.WorkOrderWorkflow';
GO
CREATE TABLE WOA.WorkOrderWorkflow
(
	WorkOrderWorkflowID [int] IDENTITY(1,1) NOT NULL PRIMARY KEY,
	WorkOrderID [int] NOT NULL 	CONSTRAINT FK_WOAWorkOrderWorkflow_WorkOrderID_WOAWorkOrder
									FOREIGN KEY (WorkOrderID)
									REFERENCES WOA.WorkOrder(WorkOrderID)
									ON DELETE CASCADE
									ON UPDATE CASCADE,
	EmployeeID [int] NULL	CONSTRAINT FK_WOAWorkOrderWorkflow_EmployeeID_Employee
								FOREIGN KEY (EmployeeID)
								REFERENCES SAP.Employee(EmployeeID),
	WorkflowStatusID [int] NOT NULL DEFAULT 1 
									CONSTRAINT FK_WOAWorkOrderWorkflow_WorkflowStatusID_WorkflowStatus
									REFERENCES WOA.WorkflowStatus(WorkflowStatusID),
	ApprovedAmount [money] NOT NULL DEFAULT 0,
	Revision [tinyint] NOT NULL DEFAULT 0,
	CreatedBy [nvarchar](256) NOT NULL DEFAULT USER_NAME(),
	CreatedDate datetime2 NOT NULL DEFAULT SYSUTCDATETIME(),
	rowguid [uniqueidentifier] ROWGUIDCOL NOT NULL DEFAULT NEWSEQUENTIALID (),
...
EXEC sys.sp_addextendedproperty 
	 @name=N'MS_Description', 
	 @value=N'SDG&E Work Order Workflow', 
	 @level0type=N'SCHEMA', @level0name=N'WOA', 
	 @level1type=N'TABLE',  @level1name=N'WorkOrderWorkflow';
GO
...
(
	InternalOrderID [int] IDENTITY(1,1) NOT NULL PRIMARY KEY,
	WorkOrderID [int] NOT NULL 	CONSTRAINT FK_WOAInternalOrder_WorkOrderID_WOAWorkOrder
									FOREIGN KEY (WorkOrderID)
									REFERENCES WOA.WorkOrder(WorkOrderID)
									ON DELETE CASCADE
									ON UPDATE CASCADE,
	...
	ExternalOrderNumber [nvarchar](20),
	WorkActivityID [nchar](1) NULL	CONSTRAINT FK_WOAInternalOrder_WorkActivityID_WorkActivity
									FOREIGN KEY (WorkActivityID)
									REFERENCES WOA.WorkActivity(WorkActivityID),
...
CREATE PROCEDURE WOA.mergeWorkOrder
				(
					...
					@ParentOrder [bit] = 0,
					@CommitmentProjectID [int] = 1,
					@Revision [tinyint] = 0,
					@CommonTimeStamp [datetime2] = NULL,
					@MergeWorkOrderID [int] OUTPUT
				)
				AS
				BEGIN
					SET NOCOUNT ON;
				...
				BEGIN
					...
					USING (Select	@WorkOrderID AS WorkOrderID,
									...
									@CommitmentProjectID AS CommitmentProjectID,
...
PRINT '-- SAP.mergeEmployee';
GO
CREATE PROCEDURE SAP.mergeEmployee
				(
					@EmployeeID [int] = 0,
					@SAPUserName [nvarchar](15),
					@WINUserName [nvarchar](15),
					@SQLUserName [nvarchar](50),
					@ManagerID [int],
					@CostCenterID [int],
					@JobTitle [nvarchar](50),
					@EmployeeName_First [nvarchar](50),
					@EmployeeName_Last [nvarchar](50),
				...
				BEGIN
					SET NOCOUNT ON;
					DECLARE @CostCenter [nchar](9);
					SET @CostCenter =  (SELECT CostCenter 
										FROM SAP.CostCenter 
										WHERE (ValidTo > SYSUTCDATETIME()) AND (CostCenterID = @CostCenterID));
					MERGE SAP.EMPLOYEE WITH (HOLDLOCK) AS m
					USING (Select	@EmployeeID AS EmployeeID,
									@SAPUserName AS SAPUserName,
									@WINUserName AS WINUserName,
									@SQLUserName AS SQLUserName,
									@ManagerID AS ManagerID,
									@CostCenterID AS CostCenterID,
									@CostCenter AS CostCenter,
									@JobTitle AS JobTitle,
									@EmployeeName_First AS EmployeeName_First,
									@EmployeeName_Last AS EmployeeName_Last,
									@EmailAddress AS EmailAddress,
									@PhoneNumber AS PhoneNumber,
									@Accountant AS Accountant,
									@Administrator AS Administrator
						  ) AS s ON m.EmployeeID = s.EmployeeID
					WHEN MATCHED THEN
						UPDATE 
...
PRINT '-- WOA.getWorkActivities';
GO
CREATE PROCEDURE	WOA.getWorkActivities
					(
						@flagVerbose bit = 0,
						@flagShowExpired bit = 0
					)
					...
					BEGIN
						...
						BEGIN TRY
							...
							IF (@flagVerbose = 1)
								SET @sqlString = 'SELECT *, '
							ELSE
								SET @sqlString = 'SELECT	 WorkActivityID, ' +
															'Name, '
							SET @sqlString = @sqlString +  'Name +' + CHAR(39) + ' - ' + CHAR(39) + '+ WorkActivityID + (CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + CHAR(39) + CHAR(39) + ' ELSE ' + CHAR(39) + ' {EXPIRED}' + CHAR(39) + ' END) AS Display, ' +
															'(CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + char(39) + '1' + CHAR(39) + ' ELSE ' + CHAR(39) + '2' + CHAR(39) + ' END) + Name +' + CHAR(39) + ' - ' + CHAR(39) + '+ WorkActivityID AS SortOrder '
							SET @sqlString = @sqlString + 'FROM WOA.WorkActivity '
							SET @sqlString = @sqlString + 'WHERE 1=1 '
							IF @flagShowExpired = 0
								SET @sqlString = @sqlString + ' AND WOA.WorkActivity.ValidTo > SYSUTCDATETIME() '
							SET @sqlString = @sqlString + 'ORDER BY SortOrder'
							EXEC(@sqlString)
						END TRY
						BEGIN CATCH
...
PRINT '-- SAP.getEmployees';
GO
CREATE PROCEDURE	SAP.getEmployees
					(
						@flagVerbose bit = 0,
						@flagShowExpired bit = 0
					)
					...
					BEGIN
						...
						BEGIN TRY
							...
							IF (@flagVerbose = 1)
								SET @sqlString = 'SELECT *, '
							ELSE
								SET @sqlString = 'SELECT	 EmployeeID, ' +
															'EmployeeIDString, ' +
															'EmployeeName_First + ' + char(39) + ' ' + CHAR(39) + ' + EmployeeName_Last AS Name, ' +
															'JobTitle, ' +
															'CostCenterID, ' +
															'CostCenter, ' +
															'ManagerID, '
							SET @sqlString = @sqlString +  'EmployeeName_First + ' + char(39) + ' ' + CHAR(39) + ' + EmployeeName_Last + (CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + CHAR(39) + CHAR(39) + ' ELSE ' + CHAR(39) + ' {EXPIRED}' + CHAR(39) + ' END) AS Display, ' +
															'(CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + char(39) + '1' + CHAR(39) + ' ELSE ' + CHAR(39) + '2' + CHAR(39) + ' END) + EmployeeName_First + ' + char(39) + ' ' + CHAR(39) + ' + EmployeeName_Last AS SortOrder '
							SET @sqlString = @sqlString + 'FROM SAP.Employee '
							SET @sqlString = @sqlString + 'WHERE 1=1 '
							IF @flagShowExpired = 0
								SET @sqlString = @sqlString + ' AND SAP.Employee.ValidTo > SYSUTCDATETIME() '
							SET @sqlString = @sqlString + 'ORDER BY SortOrder'
							EXEC(@sqlString)
						END TRY
						BEGIN CATCH
...
PRINT '-- WOA.getCommitmentProjects';
GO
CREATE PROCEDURE	WOA.getCommitmentProjects
					(
						@flagVerbose bit = 0,
						@flagShowExpired bit = 0
					)
					...
					BEGIN
						...
						BEGIN TRY
							...
							IF (@flagVerbose = 1)
								SET @sqlString = 'SELECT *, '
							ELSE
								SET @sqlString = 'SELECT	 CommitmentProjectID, ' +
															'Name, '
							SET @sqlString = @sqlString +  'Name + (CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + CHAR(39) + CHAR(39) + ' ELSE ' + CHAR(39) + ' {EXPIRED}' + CHAR(39) + ' END) AS Display, ' +
															'(CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + char(39) + '1' + CHAR(39) + ' ELSE ' + CHAR(39) + '2' + CHAR(39) + ' END) + Name AS SortOrder '
							SET @sqlString = @sqlString + 'FROM WOA.CommitmentProject '
							SET @sqlString = @sqlString + 'WHERE 1=1 '
							IF @flagShowExpired = 0
								SET @sqlString = @sqlString + ' AND WOA.CommitmentProject.ValidTo > SYSUTCDATETIME() '
							SET @sqlString = @sqlString + 'ORDER BY SortOrder'
							EXEC(@sqlString)
						END TRY
						BEGIN CATCH
...
PRINT '-- DPSS.getCOTypes';
GO
CREATE PROCEDURE	DPSS.getCOTypes
					(
						@flagVerbose bit = 0,
						@flagShowExpired bit = 0
					)
					...
					BEGIN
						...
						BEGIN TRY
							...
							IF (@flagVerbose = 1)
								SET @sqlString = 'SELECT *, '
							ELSE
								SET @sqlString = 'SELECT	 COTypeID, ' +
															'Name, '
							SET @sqlString = @sqlString +  'Name +' + CHAR(39) + ' - ' + CHAR(39) + '+ COTypeID + (CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + CHAR(39) + CHAR(39) + ' ELSE ' + CHAR(39) + ' {EXPIRED}' + CHAR(39) + ' END) AS Display, ' +
															'(CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + char(39) + '1' + CHAR(39) + ' ELSE ' + CHAR(39) + '2' + CHAR(39) + ' END) + Name +' + CHAR(39) + ' - ' + CHAR(39) + '+ COTypeID AS SortOrder '
							SET @sqlString = @sqlString + 'FROM DPSS.COType '
							SET @sqlString = @sqlString + 'WHERE 1=1 '
							IF @flagShowExpired = 0
								SET @sqlString = @sqlString + ' AND DPSS.COType.ValidTo > SYSUTCDATETIME() '
							SET @sqlString = @sqlString + 'ORDER BY SortOrder'
							EXEC(@sqlString)
						END TRY
						BEGIN CATCH
...
PRINT '-- WOA.getWorkflowStatuses';
GO
CREATE PROCEDURE	WOA.getWorkflowStatuses
					(
						@flagVerbose bit = 0,
						@flagShowExpired bit = 0
					)
					...
					BEGIN
						...
						BEGIN TRY
							...
							IF (@flagVerbose = 1)
								SET @sqlString = 'SELECT *, '
							ELSE
								SET @sqlString = 'SELECT	 WorkflowStatusID, ' +
															'Name, '
							SET @sqlString = @sqlString +  'Name  + (CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + CHAR(39) + CHAR(39) + ' ELSE ' + CHAR(39) + ' {EXPIRED}' + CHAR(39) + ' END) AS Display, ' +
															'(CASE WHEN ValidTo > SYSUTCDATETIME() THEN ' + char(39) + '1' + CHAR(39) + ' ELSE ' + CHAR(39) + '2' + CHAR(39) + ' END) + Name AS SortOrder '
							SET @sqlString = @sqlString + 'FROM WOA.WorkflowStatus '
							SET @sqlString = @sqlString + 'WHERE 1=1 '
							IF @flagShowExpired = 0
								SET @sqlString = @sqlString + ' AND WOA.WorkflowStatus.ValidTo > SYSUTCDATETIME() '
							SET @sqlString = @sqlString + 'ORDER BY SortOrder'
							EXEC(@sqlString)
						END TRY
						BEGIN CATCH
...
PRINT '-- WOA.getWorkorderWorkflow';
GO
CREATE PROCEDURE	WOA.getWorkorderWorkflow
					(
						@flagVerbose bit = 0,
						@flagShowExpired bit = 0,
						@inputWorkOrderID int = 0
					)
					AS
					BEGIN
						DECLARE @sqlString AS [VARCHAR](MAX)
						BEGIN TRY
							...
							IF (@flagVerbose = 1)
								SET @sqlString = 'SELECT *, '
							ELSE
								SET @sqlString = 'SELECT	 WorkOrderWorkflowID, ' +
															'(SELECT EmployeeName_First + ' + CHAR(39) + ' ' + CHAR(39) + ' + EmployeeName_Last ' +
															'FROM SAP.Employee ' +
															'WHERE SAP.Employee.EmployeeID = EmployeeID) AS Employee, ' +
															'(SELECT Name ' +
															'FROM WOA.WorkflowStatus ' +
															'WHERE WOA.WorkflowStatus = WorkflowStatusID) AS Status, ' +
															'ApprovedAmount, ' +
															'CreatedDate '
							SET @sqlString = @sqlString + 'FROM WOA.WorkflowStatus '
							SET @sqlString = @sqlString + 'WHERE 1=1 '
							IF @flagShowExpired = 0
								SET @sqlString = @sqlString + ' AND WOA.WorkflowStatus.ValidTo > SYSUTCDATETIME() '
							SET @sqlString = @sqlString + 'ORDER BY WOA.WorkflowStatus.CreatedDate'
							EXEC(@sqlString)
						END TRY
						BEGIN CATCH
							PRINT @sqlString
...
INSERT INTO WOA.StoredProcedure(SchemaName,Name, Description, InputParameterList, OutputParameterList, ReturnsRecords, CommandTypeProperty, IntendedUsage)
			...
			VALUES	('WOA','getGlobalConstants','Returns System Constants','@flagVerbose|@flagShowExpired',null,1,4,1),
					...
					('WOA','getWorkActivities','Returns Work Activity Index','@flagVerbose|@flagShowExpired',null,1,4,1),
					('SAP','getRegionCodes','Returns Region Code Index','@flagVerbose|@flagShowExpired',null,1,4,1), 
					('SAP','getEmployees','Returns Employee Index','@flagVerbose|@flagShowExpired',null,1,4,1),
					('WOA','getCommitmentProjects','Returns Commitment Project Index','@flagVerbose|@flagShowExpired',null,1,4,1),
					('DPSS','getCOTypes','Returns CO Type Index','@flagVerbose|@flagShowExpired',null,1,4,1),
					('WOA','getWorkflowStatuses','Returns Workflow Status Options','@flagVerbose|@flagShowExpired',null,1,4,1),
					('WOA','getWorkorderWorkflow','Returns Workflow for Specified Work Order','@flagVerbose|@flagShowExpired|@inputWorkOrderID',null,1,4,2);
GO

PRINT '-- WOA.WorkflowStatus';
GO
INSERT INTO WOA.WorkflowStatus (Name)
			VALUES	('Pending'),
					('Rejected'),
					('Approved'),
					('Forwarded');
...
PRINT '-- WOA.CommitmentProject';
GO
INSERT INTO WOA.CommitmentProject (Name,Notes)
			VALUES	('Work Order','Standalone Work Order / Project'),
					('Parent Order', 'Parent Order Commitment Control'),
					('Budget Code', 'Budget Code Commitment Control'),
					('CFSP/FIRE','Project Completed 12/31/2015'),
					('Calif. MASH',NULL),
					('AMI/SMP','jyu: Only include I/Os not already captured by the three budget codes: 04250, 04843 and 05542'),
					('AIS/CBM',NULL),
					('CA Solar - Electric','SO55 O&M'),
					('CA Solar - Gas','SO55 O&M'),
					('PSEP',NULL),
					('Smart Pricing Project (DPP)',NULL),
					('Fiber Optic Relay',NULL),
					('SCADA Expansion - Distribution',NULL),
					('ECO Sub',NULL),
					('Salt Creek Land Development',NULL),
					('Master Meter (MM)',NULL),
					('Critical Peak Pricing (CPP)',NULL),
					('CPD Enhancements Phase 2',NULL),
					('Install Gas Main for LN 49-28','Captured on 15557 budget code'),
					('SDG&E Desktop Refresh',NULL);
...
PRINT '-- WOA.WorkActivity';
GO
INSERT INTO WOA.WorkActivity(WorkActivityID,Name)
			VALUES	('A','Abandonment/Removal of assets'),
					('I','New Installation/Acquisition of fixed assets'),
					('O','O&M or Other'),
					('R','Replacement'),
					('S','Sale of Property'),
					('T','Transfer of assets/property');
GO
...
SET ValidTo = SYSUTCDATETIME()
FROM SAP.DistrictCode
WHERE SAP.DistrictCode.DistrictCodeID IN ('XB','XE','XM','SB','XC','XN','NE','XO','86','PS');
GO
...
PRINT '-- Common.County';
GO
UPDATE Common.COUNTY
SET ValidTo = SYSUTCDATETIME()
FROM	SAP.City RIGHT OUTER JOIN
		Common.County ON SAP.City.CountyID = Common.County.CountyID
WHERE SAP.City.CountyID IS NULL;
GO

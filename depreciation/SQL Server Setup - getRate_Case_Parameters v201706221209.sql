...
CREATE PROCEDURE	Depr.getRate_Case_Parameters
					...
					BEGIN
						...
						IF ISNULL(@Analysis_Version_ID,-1)=-1 AND ISNULL(@Raw_Data,-1)=-1
						...
						ELSE IF ISNULL(@Analysis_Version_ID,-1)<>-1 AND ISNULL(@Raw_Data,-1)=-1
							SELECT TOP (100) PERCENT 
                                p.Rate_Case_ID, 
                                Depr.Rate_Case.Rate_Case, 
                                p.Depr_Group_ID, 
                        ...
                        ELSE
                            SELECT   TOP (100) PERCENT
                                        p.Rate_Case_ID, Depr.Rate_Case.Rate_Case, 
                                        p.Depr_Group_ID, p.Depr_Method_Id, 
                                        PowerPlan.Depr_Group.Depr_Group, 
                                        PowerPlan.Depreciation_Method.Depr_Method, 
                                        p.Mortality_Curve_ID, 
                                        PowerPlan.Mortality_Curve.Mortality_Curve, 
                                        ...
                                        p.Rate, 
                                        p.Cost_of_Removal_Rate
                            FROM        Depr.Rate_Case_Parameter AS p 
                                        LEFT OUTER JOIN PowerPlan.Depreciation_Method ON p.Depr_Method_Id = PowerPlan.Depreciation_Method.Depr_Method_Id AND p.Depr_Method_Id = PowerPlan.Depreciation_Method.Depr_Method_Id 
                                        LEFT OUTER JOIN PowerPlan.Depr_Group ON p.Depr_Group_ID = PowerPlan.Depr_Group.Depr_Group_ID
                                        LEFT OUTER JOIN PowerPlan.Mortality_Curve ON p.Mortality_Curve_ID = PowerPlan.Mortality_Curve.Mortality_Curve_ID AND p.Mortality_Curve_ID = PowerPlan.Mortality_Curve.Mortality_Curve_ID 
                                        LEFT OUTER JOIN Depr.Rate_Case ON p.Rate_Case_ID = Depr.Rate_Case.Rate_Case_ID AND p.Rate_Case_ID = Depr.Rate_Case.Rate_Case_ID
                            ORDER BY    p.Rate_Case_ID, 
                                        PowerPlan.Depr_Group.Depr_Group
					END
					GO
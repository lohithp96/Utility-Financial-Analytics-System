...
CREATE	PROCEDURE [Depr].[setSolver_Model_FitCurve]
		...
		BEGIN
			...
			WHILE ((@loopMe = 1) AND (@localCounter <= @counterMax)) BEGIN
				...
				SET @otherCounter = 1;
				SET @innerLoop=1;
				WHILE  @innerLoop = 1 AND @otherCounter<100 BEGIN

						SET @alternateASL = @ASL - (@minimumIncrement*CAST(@otherCounter AS NUMERIC(22,8)));
                        WHILE @alternateASL >= @ASL BEGIN
                            SET @alternateASL = @alternateASL - @minimumIncrement
                        END
                        ...
                        SET @innerLoop = CASE WHEN @result_Current<=@result_Left AND @result_Current<=@result_Right THEN 1 ELSE 0 END;
